...
target_dir = f'{now_proj_dir}/dataset/PROMISE/labeled_src_code/'
...
for i in range(len(bug_proj_name)):
    ...
    for version in bug_proj_versions[i]:
        # 期待的xlsx文件建立
        xlsx_file = f'{target_dir}{proj_name}/{proj_name}-{version}.xlsx'
        ...
        for j in range(len(class_names)):
            ...
            if not os.path.exists(source_file):
                ...
                with open(source_file, 'r', encoding='utf-8', errors='ignore') as f:
                    res.loc[j, 'src'] = f.read()
                # print(res.loc[j, 'src'])
        # 移除非法字符
        # res = res.map(remove_illegal_characters)
        # 写入xlsx
        res.to_excel(xlsx_file, index=False, engine='xlsxwriter')
