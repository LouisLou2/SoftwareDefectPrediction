...
# 处理最终的pad填充
for i in range(len(ids_torch2D_lis)):
    rows, cols = ids_torch2D_lis[i].shape
    if cols < max_col:
        pad = torch.full((rows, max_col - cols), tokenizer.pad_token_id)
        ids_torch2D_lis[i] = torch.cat((ids_torch2D_lis[i], pad), dim=1)
# 存储
count = 0
for i in range(len(proj_names)):
    proj_name = proj_names[i]
    ...
    for j in range(len(version_list)):
        ...
        print(f'storing {target_file}: {ids_torch2D_lis[count].shape}')
